...
then
    pdb=1FJE
    echo "usage: $0 <base-directory>  <pdbid> <ref-pdb-name> <ref-mol2-name> <ref-dcd-name> <stop> <rmsd-threshold: 1., 1.5, 2., or 2.5> <eta: 2, 24, or 248>"
    echo "$0 tests/input/${pdb}/ ${pdb} complex.pdb lig_${pdb}.mol2 complexes.dcd 100 2.5 248 poser-scores.txt"
else
    ...
    then
        ...
        -numEta $nEta \
        -cutoff 20 \
        -outfile manuscript/output/features_${pdbid}_ \
        -scalar 1 \
        -molecular 1 \
        -mol2 $mol2 \
        -normalization 1 \
        ...
        # get score
        python src/rna_poser.py --classifier classifier/eta${eta}/RF_${rmsd}.predictor.pkl  --features manuscript/output/features_${pdbid}_traj1.txt --output ${outfile}
    fi
fi